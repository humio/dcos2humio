...
ZOO_SERVERS=$(for n in $(seq 1 $zookeeper_nodes); do echo -n "server.${n}=zk-${n}:2888:3888 " | sed "s/zk-${node}/0.0.0.0/g"; done)
docker run -d --restart=always --net=mesos --ip=${zookeeper_network}${node} \
  ...
  --name="zk-${node}" --hostname="zk-${node}" "zookeeper:${zk_version}" > /dev/null
echo -n "."
done
echo "DONE"
### ZOOKEEPER NODES ####################################################################################################

MESOS_ZK=zk://$(jot -s "," -w "zk-%g:2181" - 1 $zookeeper_nodes)/mesos
...
docker run -d --restart=always --net=mesos --ip=${master_network}${node} \
  ...
  --expose=5050 \
  --name="master-${node}" --hostname="master-${node}" "mesosphere/mesos-master:${mesos_version}" > /dev/null
echo -n "."
done
echo "DONE"
### MASTER NODES #######################################################################################################
...
EOF
docker run --name="mesosdns" --hostname="mesosdns" -d \
  -p 8123:8123 \
...
docker run -d --net=mesos --ip=${slave_network}${node} \
  ...
  -v /sys:/sys \
  --name="slave-${node}" --hostname="slave-${node}" "mesosphere/mesos-slave:${mesos_version}" > /dev/null
echo -n "."
done
echo "DONE"
### SLAVE NODES ########################################################################################################
...
docker run --name="repo" --hostname="repo" --net=mesos --ip=${tools_network}10 -d -v $(pwd):/usr/share/nginx/html nginx > /dev/null
echo -n "."

docker run --name="scheduler" -d -p 5005:5005 --restart=always \
  --net=mesos --ip=${tools_network}30 \
  --dns=${tools_network}21 \
  -e HUMIO_EXECUTOR_URL="http://repo/executor/target/executor-1.0-SNAPSHOT.jar" \
...
docker run --name="haproxy" --hostname="haproxy" -d -p 5050:5050 \
